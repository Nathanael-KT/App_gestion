...
import TheMenu2 from "./TheMenu2.vue";
import { useRouter } from "vue-router";
import { useCurrentUser } from "../composables/useCurrentUser";
const route = useRoute();
onMounted(() => {
  // Bloque l'accès direct aux routes bloquées (menu principal et enfants)
  if (blockedMenus.value.length > 0) {
    // Récupère tous les paths bloqués (menu + enfants)
...
const menuItems: Array<{
  ...
  },
  {
    ...
    children: [{ name: "Forum", path: "/forum" }],
    requiredRoles: ["admin", "employe", "magasinier"],
  },
  {
    name: "Paramètres",
...
<template>
  <component :is="isSuperAdmin ? TheMenu2 : 'div'" v-show="!isLoadingRoles">
